import Link from "next/link";

export default function Bookmarks() {
  return (
    <main>
      <h1 className="text-4xl font-bold">Bookmarks</h1>
      <ul>
        <li>
          <a href="https://component-party.dev/">Component Party</a>
        </li>
        <li>
          <a href="https://openbsd.amsterdam/">openbsd.amsterdam</a>
        </li>
        <li>
          <a href="https://thaer.no/">thær – eternal bond</a>
        </li>
        <li>
          <a href="https://www.alicemaz.com/">Alice Maz</a>
        </li>
        <li>
          <a href="https://sdf.org/">SDF</a>
        </li>
        <li>
          <a href="https://learnopengl.com/">Learn OpenGL</a>
        </li>
        <li>
          <a href="https://kuzuri.neocities.org/resources#jlpt">Kuzuri #jlpt</a>
        </li>
        <li>
          <a href="https://lazyfoo.net/tutorials/SDL/">LazyFoo</a>
        </li>
        <li>
          <a href="https://overthewire.org/wargames/">Over the Wire</a>
        </li>
        <li>
          <a href="http://tilde.club/">tilde.club</a>
        </li>
        <li>
          <a href="https://tilde.institute/">tilde.institute</a>
        </li>
        <li>
          <a href="https://neocities.org/">Neocities</a>
        </li>
        <li>
          <a href="https://kyledrake.com/">Kyle Drake</a>
        </li>
        <li>
          <a href="https://gohugo.io/">Hugo</a>
        </li>
        <li>
          <a href="https://harmful.cat-v.org/">cat -v harmful</a>
        </li>
        <li>
          <a href="https://vez.mrsk.me/freebsd-defaults">
            FreeBSD - a lesson in poor defaults
          </a>
        </li>
        <li>
          <a href="https://vez.mrsk.me/">@blakkheim</a>
        </li>
        <li>
          <a href="https://undeadly.org/">Undeadly</a>
        </li>
        <li>
          <a href="https://x.com/thegrugq">@thegrugq</a>
        </li>
        <li>
          <a href="https://ascii.town/">ascii.town</a>
        </li>
        <li>
          <a href="https://programming-motherfucker.com/">
            programming motherfucker
          </a>
        </li>
        <li>
          <a href="https://news.ycombinator.com/news">ycombinator</a>
        </li>
        <li>
          <a href="https://browserleaks.com/">browserleaks</a>
        </li>
      </ul>

      <h2 className="text-3xl font-bold">Worth reading</h2>
      <ul>
        <li>
          <a href="https://www.refactoringui.com/">
            Refactoring UI by Adam Wathan, Steve Schoger (2018)
          </a>
        </li>
        <li>
          <a href="https://iiif.library.cmu.edu/file/Simon_box00075_fld05959_bdl0003_doc0002/Simon_box00075_fld05959_bdl0003_doc0002.pdf">
            Epigrams on Programming by Alan J. Perlis (1922-1990)
          </a>
        </li>
      </ul>

      <h2 className="text-3xl font-bold">Friends arounds the Web</h2>
      <ul>
        <li>
          <a href="https://www.uncontrollable-consequences.com/">
            Uncontrollable Consequences
          </a>
        </li>
        <li>
          <a href="https://www.youtube.com/@Buggoesbrr">@Buggoesbrr</a>
        </li>
        <li>
          <a href="https://omeralperguzel.github.io/">Ömer Alper Güzel</a>
        </li>
      </ul>

      <footer>
        <Link href="/lisp">Also check my Common Lisp page here</Link>
      </footer>
    </main>
  );
}
